      ...
      <div className="relative mx-auto max-w-5xl">
        ...
        <div className="max-w-3xl">
          ...
          <p className="mt-6 text-base sm:text-lg text-gray-600">
            Not just code — I focus on usability, clarity, and long-term maintainability. Every project is built with intention.
          </p>
        </div>
        ...
        <div className="mt-20 sm:mt-24 md:mt-28 grid gap-12 sm:gap-16 md:grid-cols-3">
          {[
            ...
            <div key={item.title} className="relative">
              ...
              <h3 className="text-lg sm:text-xl font-semibold text-[#272727]">{item.title}</h3>
              <p className="mt-2 sm:mt-3 text-gray-600 text-sm sm:text-base">{item.text}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-20 sm:mt-28 space-y-8 sm:space-y-12">
          {services.map((service, i) => (
            ...
            >
              ...
              <h2 className="text-xl sm:text-2xl font-semibold text-[#272727]">{service.title}</h2>
              <p className="mt-3 sm:mt-4 max-w-2xl text-gray-600 text-sm sm:text-base">{service.description}</p>
              <div className="mt-4 sm:mt-6 flex flex-wrap gap-2">
                {service.tech.map((t) => (
                  <span key={t} className="rounded-full border border-[#FD853A]/40 px-3 sm:px-4 py-1 text-xs sm:text-sm text-[#FD853A]">
                    {t}
                  </span>
                ))}
              </div>
        ...
        <div className="mt-20 sm:mt-28">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#272727]">My process</h2>
          <div className="mt-8 sm:mt-10 grid gap-6 sm:gap-8 md:grid-cols-4">
            {["Plan", "Design", "Build", "Iterate"].map((step, i) => (
              <div key={step} className="relative">
                <span className="text-sm sm:text-base font-semibold text-[#FD853A]">Step {i + 1}</span>
                <h3 className="mt-1 sm:mt-2 text-lg sm:text-xl font-semibold text-[#272727]">{step}</h3>
        ...
        <div className="mt-20 sm:mt-28 rounded-3xl bg-[#272727] px-6 sm:px-10 py-12 sm:py-16 text-center text-white">
          <p className="text-xs sm:text-sm uppercase tracking-widest text-[#FD853A]">Proof over promises</p>
          <h2 className="mt-2 sm:mt-4 text-2xl sm:text-4xl font-extrabold">Don’t believe me?</h2>
          <p className="mt-2 sm:mt-4 text-gray-300 text-sm sm:text-base">
            That’s totally fair. Take a look at my real projects and judge the quality yourself.
          </p>
          <a
            href="/portfolio"
        ...
        <div className="mt-16 sm:mt-32 text-center">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#272727]">Want to work together?</h2>
